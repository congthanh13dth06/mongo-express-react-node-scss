...
import ScheduleHtml from '../../../../../ui/public/register/new/schedule/schedule.html'
...
export default class ScheduleComponent extends Component {
  ...
  handleResponseCode() {
    AppServices.services.logger.info('ScheduleComponent excute handleResponseCode')
    try {
      const{handleSetViewNew, viewNew} = this.props
      AppServices.services.cacheLayout.clear()
      handleSetViewNew(viewNew.noti)
    } catch (e) {
  ...
  handleOnBlur(event) {
    AppServices.services.logger.info('ScheduleComponent excute handleOnBlur')
    try {
      const { value, name } = event.target;
  ...
  handleSkip(event) {
    AppServices.services.logger.info('ScheduleComponent excute handleSkip')
    try {
      event.preventDefault()
      this.state.timeout.setTimeout()
      this.handleProcess()
    } catch (e) {
      AppServices.services.logger.error(`ScheduleComponent handleSkip`)
      AppServices.services.logger.error(`Field ${e.field} Message ${e.message}`)
      this.state.timeout.setTimeout(false, e.field, e.message)
      this.handleProcess(false)
    }
  ...
  handleRequest() {
    ...
    try {
      ...
      const { match, viewNew,view } = this.props;

      const dataCacheMain = AppServices.services.cacheLayout.get(match.url, view.main);
      const dataCacheInputPhone = AppServices.services.cacheLayout.get(match.url, view.new, viewNew.inputPhone);
      ...
      data.set('fullName', dataCacheMain.fullName, data.fullName)
      data.set('phoneNumber', dataCacheInputPhone.phoneNumber, data.phoneNumber)
      data.set('identityNo', dataCacheMain.identityNo, data.identityNo)
      ...
      AppServices.services.logger.trace(`Field ${e.field} Message ${e.message}`)
      this.state.timeout.setTimeout(false, e.field, e.message)
      this.handleProcess(false)
    }
  }
  ...
  render() {
    ...
    const { model, data, timeout, ui } = this.state;
    const { localizeKey, viewNew, handleSetViewNew } = this.props;
