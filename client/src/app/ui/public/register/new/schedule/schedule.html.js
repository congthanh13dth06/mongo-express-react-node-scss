...
export default class ScheduleHtml extends Component {
  ...
  render() {
    ...
    const { appServices, localizeKey, model, data, timeout, ui, viewNew, handleSetViewNew, handleRedirect, handleRefLineProcress, handleOnChange, handleOnBlur, handleTimeout, handleSkip } = this.props;

    return (
      <div className={Style.REGISTER}>
        <div className={Style.HEADER}>
          <div>
            <a href="javascript:" className={`${Style.HEADER_ICON_LEFT} ${timeout.status ? Style.DISABLED : ''}`} onClick={(event) => {event.preventDefault(); handleSetViewNew(viewNew.inputAccount)}}>{Svg.BACK.content}
        ...
        <div className={Style.FORM_FOGOT}>
          <form>
            ...
            <div className={Style.SKIP}>
              <p className={Style.TEXT_OR}>{appServices.services.localize.getLocalize(localizeKey.REGISTER_OR_WILL_CONTACT)}</p>
              <div style={{"textAlign": "center"}}>
                <a className={`${Style.BUTTON} ${(timeout.status) ? Style.DISABLED : ''}`} href="" onClick={handleSkip}>
                  {appServices.services.localize.getLocalize(localizeKey.REGISTER_BUTTON_SKIP)}
                </a>
              </div>
              <p className={Style.TEXT}>
                {appServices.services.localize.getLocalize(localizeKey.REGISTER_WILL_CONTACT_ONE)}
                <span>{appServices.services.localize.getLocalize(localizeKey.REGISTER_WILL_CONTACT_TWO)}</span>
                {appServices.services.localize.getLocalize(localizeKey.REGISTER_WILL_CONTACT_THREE)}
              </p>
            </div>
            <Button.primary
              style={{width: '100%'}}
